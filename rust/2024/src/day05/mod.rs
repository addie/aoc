...
fn is_valid_x_mas(grid: &[Vec<char>], r: usize, c: usize) -> bool {
    // only check 'A' values that aren't in the first or last row or col.
    if grid[r][c] != 'A' || r < 1 || c < 1 || r > grid.len() - 2 || c > grid[r].len() - 2 {
        return false;
    }
    let diag1 = (grid[r - 1][c - 1], grid[r + 1][c + 1]);
    let diag2 = (grid[r - 1][c + 1], grid[r + 1][c - 1]);
